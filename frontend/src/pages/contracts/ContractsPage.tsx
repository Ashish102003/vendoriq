...
const SORTABLE_COLUMNS: { key: ContractSortField; label: string }[] = [
...
interface PageData {
  items: import('../../types').ContractListItem[]
  total: number
  total_pages: number
}
...
export function ContractsPage() {
  ...
  const canAdd = canManageContracts(roleName)
  ...
  const [sortBy, setSortBy] = useState<ContractSortField>('created_at')
  ...
  useEffect(() => {
    ...
    vendorsApi
      ...
      .catch(() => {
        // vendor filter dropdown degrades gracefully
      })
    return () => {
      active = false
    }
  ...
  }, [page, debouncedSearch, vendorId, statusFilter, activeFilter, sortBy, sortOrder, reloadKey])
  ...
  const sortedStats = useMemo(
    () => [
      { label: 'Total', value: stats.total_contracts, className: 'text-slate-100' },
      { label: 'Active', value: stats.active_contracts, className: 'text-emerald-400' },
      { label: 'Completed', value: stats.completed_contracts, className: 'text-sky-300' },
      { label: 'On Hold', value: stats.on_hold_contracts, className: 'text-orange-400' },
      {
  ...
  function toggleSort(key: ContractSortField) {
    if (sortBy === key) {
  ...
  function renderSortableHeader({
    ...
    column: { key: ContractSortField; label: string }
    className?: string
  }) {
    return (
  ...
  return (
    <AppLayout title="Contracts">
      <PageHeader
        title="Contracts"
        subtitle="Manage vendor agreements and contractual engagements."
        actions={
          canAdd ? (
          ...
          <EmptyState
            ...
            message={
              hasFilters
                ...
                : 'Create your first vendor contract to begin tracking contractual engagements.'
            }
            actions={
              canAdd && !hasFilters ? (
                <Button onClick={() => navigate('/contracts/new')}>
              ...
              <table className="w-full text-left text-sm">
                <thead className="border-b border-slate-800 bg-slate-800/30">
                  <tr>
                    ...
                    {renderSortableHeader({ column: SORTABLE_COLUMNS[5] })}
